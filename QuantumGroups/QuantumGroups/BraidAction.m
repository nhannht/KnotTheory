...
\!\(\(ExpandReducedPowers[\[CapitalGamma]_]\)[F_] := \(F /. \[IndentingNewLine]{ReducedPower[\(X\_i_\^+\), n_] :> With[{d = \(CartanFactors[\[CapitalGamma]]\)\[LeftDoubleBracket]i\[RightDoubleBracket]}, NonCommutativePower[\(X\_i\^+\), n]\/\(qFactorial[n]\)[q\^d]], \[IndentingNewLine]ReducedPower[\(X\_i_\^-\), n_] :> With[{d = \(CartanFactors[\[CapitalGamma]]\)\[LeftDoubleBracket]i\[RightDoubleBracket]}, NonCommutativePower[\(X\_i\^-\), n]\/\(qFactorial[n]\)[q\^d]]}\) /. OrderingRules[\[CapitalGamma]]\)

BraidAction[\[CapitalGamma]_][{word___},0]:=0

\!\(\(BraidAction[\[CapitalGamma]_]\)[{T\_i_}, \(X\_j_\^+\)] := \(\(BraidAction[\[CapitalGamma]]\)[{T\_i}, \(X\_j\^+\)] = \[IndentingNewLine]If[i == j, \(-\(X\_i\^-\) ** K\_i\), With[{a = \(CartanMatrix[\[CapitalGamma]]\)\[LeftDoubleBracket]i, j\[RightDoubleBracket], d = \(CartanFactors[\[CapitalGamma]]\)\[LeftDoubleBracket]i\[RightDoubleBracket]}, \(ExpandReducedPowers[\[CapitalGamma]]\)[\[Sum]\+\(r = 0\)\%\(-a\)\(\((\(-1\))\)\^\(r - a\)\) \(q\^\(\(-d\)\ r\)\) ReducedPower[\(X\_i\^+\), \(-a\) - r] ** \(X\_j\^+\) ** ReducedPower[\(X\_i\^+\), r]]]]\)\)

\!\(\(BraidAction[\[CapitalGamma]_]\)[{T\_i_}, \(X\_j_\^-\)] := \(\(BraidAction[\[CapitalGamma]]\)[{T\_i}, \(X\_j\^-\)] = \[IndentingNewLine]If[i == j, \(-K\_i\^\(-1\) ** \(X\_i\^+\)\), With[{a = \(CartanMatrix[\[CapitalGamma]]\)\[LeftDoubleBracket]i, j\[RightDoubleBracket], d = \(CartanFactors[\[CapitalGamma]]\)\[LeftDoubleBracket]i\[RightDoubleBracket]}, \(ExpandReducedPowers[\[CapitalGamma]]\)[\[Sum]\+\(r = 0\)\%\(-a\)\(\((\(-1\))\)\^\(r - a\)\) \(q\^\(d\ r\)\) ReducedPower[\(X\_i\^-\), r] ** \(X\_j\^-\) ** ReducedPower[\(X\_i\^-\), \(-a\) - r]]]]\)\)
...
\!\(OrderingRules[\[CapitalGamma]_] := \(OrderingRules[\[CapitalGamma]] = With[{d = CartanFactors[\[CapitalGamma]], a = CartanMatrix[\[CapitalGamma]]}, {\[IndentingNewLine]K\_i_ ** K\_i_\^\(-1\) :> \[ScriptOne], \[IndentingNewLine]K\_i_\^\(-1\) ** K\_i_ :> \[ScriptOne], \[IndentingNewLine]Y___ ** K\_i_\^n_.  ** K\_j_\^m_.  ** Z___ /; i > j :> Y ** K\_j\^m ** K\_i\^n ** Z, \[IndentingNewLine]\(X\_j_\^+\) ** K\_i_\^n_.  :> \(q\^\(\(-n\)\ d\[LeftDoubleBracket]i\[RightDoubleBracket] a\[LeftDoubleBracket]i, j\[RightDoubleBracket]\)\) K\_i\^n ** \(X\_j\^+\), \[IndentingNewLine]K\_i_\^n_.  ** \(X\_j_\^-\) :> \(q\^\(\(-n\)\ d\[LeftDoubleBracket]i\[RightDoubleBracket] a\[LeftDoubleBracket]i, j\[RightDoubleBracket]\)\) \(X\_j\^-\) ** K\_i\^n, \[IndentingNewLine]\(X\_i_\^+\) ** \(X\_j_\^-\) :> \(X\_j\^-\) ** \(X\_i\^+\) + DiscreteDelta[i - j] \(K\_i - K\_i\^\(-1\)\)\/\(q\^d\[LeftDoubleBracket]i\[RightDoubleBracket] - q\^\(-d\[LeftDoubleBracket]i\[RightDoubleBracket]\)\)\[IndentingNewLine]}]\)\)





\!\(ExtraOrderingRules[\[CapitalGamma]_] := \(ExtraOrderingRules[\[CapitalGamma]] = With[{d = CartanFactors[\[CapitalGamma]], a = CartanMatrix[\[CapitalGamma]]}, {Y___ ** \(X\_i_\^+\) ** \(X\_j_\^+\) ** Z___ /; \((i < j && a\[LeftDoubleBracket]i, j\[RightDoubleBracket] == 0)\) :> Y ** \(X\_j\^+\) ** \(X\_i\^+\) ** Z, \[IndentingNewLine]Y___ ** \(X\_i_\^-\) ** \(X\_j_\^-\) ** Z___ /; \((i < j && a\[LeftDoubleBracket]i, j\[RightDoubleBracket] == 0)\) :> Y ** \(X\_j\^-\) ** \(X\_i\^-\) ** Z}]\)\)
...
\!\(BraidRelations[\[CapitalGamma]_] := Module[{m = CartanMatrix[\[CapitalGamma]] Transpose[CartanMatrix[\[CapitalGamma]]] /. {\(n_?# >= 4 &\) :> \[Infinity], 3 -> 6, 2 -> 4, 1 -> 3, 0 -> 2}, w}, \[IndentingNewLine]w[i_, j_, n_] := Take[{T\_i, T\_j, T\_i, T\_j, T\_i, T\_j}, n]; DeleteCases[Flatten[Table[If[m\[LeftDoubleBracket]i, j\[RightDoubleBracket] < \[Infinity], w[i, j, m\[LeftDoubleBracket]i, j\[RightDoubleBracket]] == w[j, i, m\[LeftDoubleBracket]i, j\[RightDoubleBracket]], True], {i, 1, Rank[\[CapitalGamma]]}, {j, i, Rank[\[CapitalGamma]]}]], True]\[IndentingNewLine]]\)

CheckBraidRelation[\[CapitalGamma]_][word1_==word2_]:=
  And@@Simplify[
      BraidAction[\[CapitalGamma]][word1,#]==
